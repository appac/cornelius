...
class TeamRequest extends MlbRequest {
    ...
    buildUrl(options) {
        ...
        case ACTIONS.LEADERS:
            uri = `${STATS_API.BASE}/teams/${options.id}/leaders?leaderCategories=${options.metric}&season=${options.season}&leaderGameTypes=${options.game_type}&limit=${options.limit}`;
            break;
        case ACTIONS.ROSTER:
        ...
        case ACTIONS.TRANSACTIONS:
            ...
            break;
        case ACTIONS.NEWS:
            uri = `${WSFB.BASE}${WSFB.NEWS}.bam?team_id='${options.id}'&wsfb_news_browse.recPP=${options.limit}&wsfb_news_browse.recSP=1`;
            break;
