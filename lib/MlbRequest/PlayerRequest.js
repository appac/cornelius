...
const CONSTANTS = require('./CONSTANTS').PLAYER;
...
class PlayerRequest extends MlbRequest {
    ...
    buildUrl(options) {
        ...
        case ACTIONS.STATS:
            ...
            uri = `${PUBLIC_LOOKUP.BASE}${statEndpoint}.bam?player_id=${options.id}&league_list_id='mlb_hist'&${statEndpoint}.season=${options.stat_season}&game_type='${options.stat_game_type}'`;
            break;
        case ACTIONS.INJURIES:
            uri = `${WSFB.BASE}${WSFB.INJURIES}.bam`;
            break;
        case ACTIONS.NEWS:
            uri = `${WSFB.BASE}${WSFB.NEWS}.bam?player_id='${options.id}'`;
            break;
