...
class MlbRequest extends EventEmitter {
    ...
    buildUrl(options) {
        ...
        case ACTIONS.STATS:
            ...
            uri = `${BASE_URLS.PUBLIC_LOOKUP}${statEndpoint}.bam?player_id=${options.id}&league_list_id='mlb_hist'&${statEndpoint}.season=${options.season}&game_type='R'`;
            break;
        default:
        ...
        //     uri = `${CONFIG.PUBLIC_LOOKUP_BASE_URL}/named.search_player_all.bam?sport_code='mlb'&name_part='${options.query}%25'&active_sw='${options.active ? 'Y' : 'N'}'`;
        // } else if (this.endpoint === 'stats') {
        //     uri = `${CONFIG.PUBLIC_LOOKUP_BASE_URL}/named.sport_${this.role}_tm.bam`;
        // } else {
        //     switch (this.endpoint) {
        //     case 'details':
        //         uri = `${STATS_API_BASE_URL}/people/?personIds=${options.id}`;
        //         break;
        //     default:
        //         this.emit('error', 'No endpoint provided.');
        //         break;
